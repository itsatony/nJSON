...
var postIndex = fs.readFileSync(__dirname + '/../public/post.html', 'utf8');
...
function router(req, res, next) {
	...
	} else if (route === 'post.html') {
		return getJSON(req, res, next, postIndex);	
	}
	return next();
};
...
function cleanUID(uid) {
	if (typeof uid !== 'string' || uid.length < 6) {
		uid = minions.randomString(7, true, true, true);
	}
...
function getJSON(req, res, next, template) {
	var uid = cleanUID(req.query.id);
	// if (isUID(uid) === false) {
	...
	return getKeyContent(
		uid,
		function(err, data) {
			var html = renderIndex(data, uid, template);
...
function getKeyContent(uid, callback) {
	...
	var getResult = redisClient.get(config.redis.prefix + uid, callback);
	redisClient.expire(config.redis.prefix + uid, config.redis.ttl);
	return getResult;
};
...
function setKeyContent(uid, content, callback) {
	...
	var setResult = redisClient.set(config.redis.prefix + uid, content, callback);
	redisClient.expire(config.redis.prefix + uid, config.redis.ttl);
	return setResult;
};
...
function renderIndex(jsonData, uid, template) {
	// TODO: regex or dot this!
	template = template || index;
	var html = template.replace(/\{\{JSONDATA\}\}/g, jsonData);
	html = html.replace(/\{\{UID\}\}/g, uid);
	html = html.replace(/\{\{totalSets\}\}/g, totalSets);
	html = html.replace(/\{\{totalGets\}\}/g, totalGets);
	return html;
};
